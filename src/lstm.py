...
class LSTMBase(object):
    def __init__(self, batch_size=24, hidden_units=64, timesteps=250,
        ...
        self.w_i, self.w_j, self.w_f, self.w_o = tf.split(self.cell_weight, 4, 1)
        self.b_i, self.b_j, self.b_f, self.b_o = tf.split(self.cell_bias,   4, 0)

        self.data = tf.nn.embedding_lookup(self.embed, self.x_ph)
        self.data = tf.transpose(self.data, [1, 0, 2])

        c_t = tf.zeros([self.batch_size, self.hidden_units], dtype=tf.float32)
        h_t = tf.zeros([self.batch_size, self.hidden_units], dtype=tf.float32)
        self.cell_state = [c_t]
        self.history    = [h_t]

        for step in range(self.timesteps):
            inputs = tf.concat([self.data[step], h_t], 1)

            i_t = tf.sigmoid(tf.add(tf.matmul(inputs, self.w_i), self.b_i))
            j_t = tf.tanh(tf.add(tf.matmul(inputs, self.w_j), self.b_j))
            f_t = tf.sigmoid(tf.add(tf.matmul(inputs, self.w_f), self.b_f))
            o_t = tf.sigmoid(tf.add(tf.matmul(inputs, self.w_o), self.b_o))

            pre = tf.multiply(f_t, c_t)
            cur = tf.multiply(i_t, j_t)
            c_t = tf.add(pre, cur)
            h_t = tf.multiply(o_t, tf.tanh(c_t))

            self.cell_state = tf.concat([self.cell_state, [c_t]], 0)
        ...
        self.cell_state = tf.transpose(self.cell_state, [1, 0, 2])
...
class LSTM(LSTMBase):
    ...
    def get_weights(self):
        cell_weight = tf.get_variable('cell_weight', [self.embed_shape[1] + self.hidden_units,
                                                      4 * self.hidden_units])
        cell_bias   = tf.get_variable('cell_bias',   [4 * self.hidden_units])
...
class LSTMDual(LSTMBase):
    def __init__(self, weight_path, batch_size=24, hidden_units=64, timesteps=250,
        ...
        perturb_i = self.get_perturb((0, self.embed_shape[1]), faulty_dim,
                                     self._epsilon, seed)
        perturb_c = self.get_perturb((self.embed_shape[1],
                                      self.embed_shape[1] + self.hidden_units),
                                     faulty_dim, self._lambda, seed)
        perturb_h = self.get_perturb((self.embed_shape[1] + self.hidden_units,
                                      len(self.diverge)),
                                     faulty_dim, self._lambda, seed)
        ...
        c_t_ptb  = tf.zeros([self.batch_size, self.hidden_units], dtype=tf.float32)
        h_t_ptb  = tf.zeros([self.batch_size, self.hidden_units], dtype=tf.float32)
        self.history_ptb = [h_t_ptb]

        for step in range(self.timesteps):
            c_t_ptb = tf.add(c_t_ptb, perturb_c)
            h_t_ptb = tf.add(h_t_ptb, perturb_h)
            inputs = tf.concat([self.data_ptb[step], h_t_ptb], 1)
            ...
            pre = tf.multiply(f_t, c_t_ptb)
            cur = tf.multiply(i_t, j_t)
            c_t_ptb = tf.add(pre, cur)
            h_t_ptb = tf.multiply(o_t, tf.tanh(c_t_ptb))
